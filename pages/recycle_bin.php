...
$cleanup = $pdo->prepare("DELETE FROM products WHERE status = 'deleted' AND deleted_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
$cleanup->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($product) {
        if ($action === 'restore') {
            $upd = $pdo->prepare("UPDATE products SET status = 'active', deleted_at = NULL WHERE id = ?");
...
<div class="container py-12">
    <div class="flex flex-col md-flex-row justify-between items-start md-items-center mb-10 gap-4">
        <div>
            <h1 class="mb-2" style="font-size: 2.5rem; font-weight: 900;">Recycle Bin</h1>
    ...
    <?php if (empty($deletedProducts)): ?>
        <div class="card p-12 text-center" style="border-radius: 24px; border: 2px dashed var(--border-light); background: transparent;">
    ...
    <?php else: ?>
        <div class="grid grid-cols-1 md-grid-cols-2 lg-grid-cols-3 gap-8">
            <?php foreach ($deletedProducts as $prod): 
                $deletedDate = new DateTime($prod['deleted_at']);
                $expiryDate = (clone $deletedDate)->modify('+30 days');
                $now = new DateTime();
            ...
            ?>
                <div class="card overflow-hidden flex flex-col" style="border-radius: 20px; border: 1px solid var(--border-light); transition: all 0.3s ease;">
                    <div class="relative h-48 bg-slate-100">
                        ...
                        <div class="absolute top-4 right-4 bg-white/95 backdrop-blur px-3 py-1.5 rounded-full text-[0.65rem] font-black shadow-lg <?php echo $daysLeft < 5 ? 'text-red-600' : 'text-slate-600'; ?>" style="letter-spacing: 0.08em; border: 1px solid rgba(0,0,0,0.05);">
                            <?php echo $daysLeft; ?> DAYS REMAINING
                        </div>
                    </div>
                    
                    <div class="p-6 flex-grow">
                        <span class="text-[0.65rem] font-black uppercase text-indigo-500 mb-2 block" style="letter-spacing: 0.15em; opacity: 0.8;"><?php echo sanitize($prod['category_name']); ?></span>
                        ...
                        <div class="flex items-center gap-3">
                            <form method="post" style="flex: 1;">
                                ...
                                <button type="submit" class="btn btn-outline w-full py-3 hover-scale" style="border-radius: 14px; font-weight: 800; border-color: var(--primary); color: var(--primary); font-size: 0.9rem;">
                                    Restore Item
                                </button>
                            </form>
                            <form method="post" onsubmit="return confirm('Delete this listing forever? This action cannot be undone.')">
                                ...
                                <button type="submit" class="btn btn-danger py-3 px-5 hover-scale shadow-sm" style="border-radius: 14px; background: #ef4444; color: #fff; border: none; display: flex; align-items: center; justify-content: center;">
